...
defimpl QueryAnalyser, for: PgQuery.AlterTableStmt do
  ...
  defp analyse_alter_table_cmd(_cmd, %{electrified?: false} = analysis, _opts) do
    {:halt, analysis}
  end
...
defimpl QueryAnalyser, for: PgQuery.RenameStmt do
  def analyse(stmt, %{electrified?: false} = analysis, _opts) do
    %{analysis | action: action(stmt)}
  end

  def analyse(stmt, %{electrified?: true} = analysis, _opts) do
    %{
      analysis
      | action: action(stmt),
        allowed?: false,
